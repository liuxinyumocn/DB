...
public class Table extends Chain{

	private int start = 0;
	private int endIndex = 0;
	private String table = null;
	
	public Table(String sql , int startIndex) throws Exception {
		super(sql,"table");
	...
	private void parse() throws Exception {
		...
		if(index == -1) {
			//未找到
			throw new Exception("未能找到FROM字段");
		}
		//判断该字符起是否是FROM
		int indexFrom = this.sql.indexOf("FROM", index);
		if(indexFrom != index)
			throw new Exception("未能找到FROM字段");
	...
	public Chain[] nextChain() throws Exception {
		...
		int finished = 0;
		try{
			Where where = new Where(this.sql,endIndex+1);
			chains[finished++] = where;
		}catch(Exception e){
			//没有Where
			//e.printStackTrace();
		}
		try{
			Limit limit = new Limit(this.sql,endIndex+1);
			chains[finished++] = limit;
		}catch(Exception e){
			//没有Limit
		}
//		try{
